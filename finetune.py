...
add_arg(
    "max_audio_len", type=float, default=30, help="最大的音频长度，单位秒，不能大于30秒"
)
...
add_arg(
    "push_to_hub", type=bool, default=False, help="是否将模型权重推到HuggingFace Hub"
)
...
def main():
    ...
    if len(processor.tokenizer.get_vocab()) > model.config.vocab_size:
        print(
            f"📈 詞彙表已擴展：{model.config.vocab_size} -> {len(processor.tokenizer.get_vocab())}"
        )
    ...
    if args.resume_from_checkpoint and not args.override_training_args:
        ...
        # 覆盖checkpoint中的训练参数，只加载模型权重和优化器状态
        print(f"⚠️  覆盖checkpoint中的训练参数，使用当前设定的参数")
        print(f"📂 从 {args.resume_from_checkpoint} 加载模型权重")
        trainer.train(resume_from_checkpoint=args.resume_from_checkpoint)
    else:
        # 从头开始训练
        trainer.train()
